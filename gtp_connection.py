...
class GtpConnection:
    ...
    def genmove_cmd(self, args):
        ...
        signal.alarm(self.timelimit_value)

        try:
            board_copy = self.board.copy()
            t_table = {}
            board_copy.current_player = color
            win = self.negamax(board_copy, t_table)
            if win:
                move = self.best_move
            if not win:
    ...
    def solve_cmd(self, args):
        ...
        win = None
        self.best_move = None
        self.toPlay = self.board.current_player
        winner = None
        def handler(signum, frame):
            raise TimeoutError
        ...
        if win:
            move_coord = point_to_coord(self.best_move, self.board.size)
            self.best_move = format_point(move_coord).lower()
            winner = self.int_to_color(self.toPlay)
            self.respond("{} {}".format(winner, self.best_move))
            return
        if not win:
            opponent = GoBoardUtil.opponent(self.toPlay)
            winner = self.int_to_color(opponent)
            self.respond(winner)
            return


    def int_to_color(self, int):
        if int == 1:
            return 'b'
        if int == 2:
            return 'w'

    # Implementation of negamax using a transposition table
    # This code is adapted from boolean_negamax_tt.py provided in 
    # the CMPUT455 sample code
    def negamax(self, board, t_table):
        code = self.generate_code(board)
        result = t_table.get(code)

        if result != None:
            return result

        board_color = board.current_player
        legal_moves = GoBoardUtil.generate_legal_moves(board, board.current_player)
        if len(legal_moves) == 0:
            return self.store_result(t_table, code, False)

        for move in legal_moves:
            board.play_move(move, board_color)
            win = not self.negamax(board, t_table)
            board.undo_move(move)

            if win:
                self.best_move = move
                return self.store_result(t_table, code, True)
        
        return self.store_result(t_table, code, False)
    
    def store_result(self, t_table, code, result):
        t_table[code] = result
        return result
       
    def generate_code(self, board):
        code = 0
        twoD_board = GoBoardUtil.get_twoD_board(board)
        for x in range(board.size):
            for y in range(board.size):
                code = 3 * code + twoD_board[x][y]
        return code

    def time_limit(self, args):
        """timelimit args[0] is an integer for max time """
        if len(args) > 0:
            self.timelimit_value = int(args[0])
            self.respond(self.timelimit_value)
